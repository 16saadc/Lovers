...
public class MateFinder {
    ...
    public void adjustEmotionsBasedOnLearning() {
        ...
        int highTraitCount = 0;
        int mediumTraitCount = 0;

        //count high important traits
        if (Main.confidenceImportance > 3 && potentialMate.getConfidence().getLevel() == Level.HIGH) highTraitCount++;
        if (Main.intelligenceImportance > 3 && potentialMate.getIntelligence().getOverallLevel() == Level.HIGH) highTraitCount++;
        if (Main.auraImportance > 3 && potentialMate.getAura() == Aura.BOOSTS_MY_EGO) highTraitCount++;
        if (Main.physicalAttractionImportance > 3 && potentialMatePhysical == Level.HIGH) highTraitCount++;
        if (Main.personalityImportance > 3 && potentialMatePersonality == Level.HIGH) highTraitCount++;

        if (highTraitCount > 4) {
            System.out.println("This mate has 4 or more traits that I really like, so I am more impressed and excited");
            Main.updateEmotion(EmotionalState.IMPRESSED, Level.HIGH);
            Main.updateEmotion(EmotionalState.EXCITED, Level.MEDIUM);
        } else if (highTraitCount > 2) {
            System.out.println("This mate has 2-3 traits that I really like, so I am more impressed and excited");
            Main.updateEmotion(EmotionalState.IMPRESSED, Level.MEDIUM);
            Main.updateEmotion(EmotionalState.EXCITED, Level.LOW);
        } else if (highTraitCount > 0) {
